...
Josh briefly demoed Claude Code's [multi-agent mode](https://docs.anthropic.com/en/docs/claude-code/sub-agents#choose-between-subagents-and-main-conversation), currently in beta. Main conversation plus n sub-agent panes in one terminal. There was an animation of making a change and watching it ripple through. The audience reaction was telling: awe, but also unease. There's a real payoff here. The sophistication is geared toward getting more out of the system. But even so, that unease is a desire line, the same instinct that pushes most people toward GUIs over terminals. Most users will want to be pushed away from this complexity, not into it.
