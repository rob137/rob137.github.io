...
SDD forces carefulness up front. It gives you a legible banner in corporate environments for practices you probably should have been following anyway, rather than racing toward arbitrary deadlines. When the spec is the thing the machine reads, writing a good spec stops being overhead and starts being the work. LLMs will only really do what we ask them to, and can only infer so much when what we give them is incomplete. Anything that gets teams spending more time thinking about what they actually want and what the words they're using actually mean in a bounded context is good.

But look at who's pushing SDD loudest. It tends to be architects at large organisations where a neat, VP-legible picture matters. The rough-and-ready, bottom-up end of the AI-tools world — Karpathy, Simon Willison, swyx — haven't picked up spec-driven development as a named practice at all. They're just building, with tight feedback loops and direct control over what the model sees. That split is worth paying attention to.

**The tooling problem.** Tools that formalise SDD, like Amazon's Kiro, introduce an intermediary layer between you and the model. When you run a `/specify` command, text gets added to the context window that you don't control and can't see. If you're already comfortable with LLMs, that's noise. If you're not, you're learning two things at once: how the model works and how the tool works, with an opaque layer between them. A tight grip on what's in the context window is the difference between knowing what you're doing and watching something happen on your screen. And even when you do know what the model's been told, people's mental models of LLM reliability vary wildly. I've seen the full range among experienced users: some assume the model will pick up anything in a spec, others assume it'll pick up more or less nothing. The truth moves depending on which model you're using, how much context it's holding, how far through a session you are. Small differences in fidelity compound fast. A 99% reliable executor and a 96% reliable executor feel similar on any given step. Over fifty steps, the 99% executor still has a 60% chance of getting everything right. The 96% executor is down to 13%. Over a hundred steps: 37% versus 2%. Three percentage points per step, and one executor is twenty times more likely to get the job done.

There's an awful lot of intellectual heavy lifting going into SDD frameworks right now: specification formats, getting the right tone and structure, not over-specifying, tracking changes over time, respecting derived specs that flow from your original. That work is meaty and deserves human attention. But the difference between a frontier model and last year's model can feel trivial by comparison — and because of compounding, it isn't. Teams unfamiliar with LLMs will often try to cut costs by using older or smaller models. This is penny wise and pound foolish. Mistakes breed indirection. Wrong code gets treated as authoritative by the next model to join the project, and every new conversation is effectively a new joiner reading the codebase for the first time. Bad code in the repo becomes the training signal for the next round of generation. This affects time to market, iteration speed, bug rates, how many people you need to hire. The hidden costs of a cheaper model dwarf the visible savings.

**Spec rot.** Markdown specs are loosely coupled to the code, but they're also loosely coupled to the conversations that produced them. In most teams, the canonical document is what people actually said in the meeting. Tickets are a lossy shadow of that. A spec file in a repo or a Confluence page is another degree of indirection further out. Someone has to remember to update it, and it doesn't take much drift before people stop trusting it. I've seen this happen on projects — the specs exist, they're just wrong enough that nobody checks them any more. One missed update and they start to rot. When something consistently goes wrong because of human error, it's worth asking whether you're witnessing error or nature. If people reliably forget to update the spec, is the answer more rules and process, putting the responsibility on the person who forgot? Or does it reveal a [Don't Make Me Think](https://en.wikipedia.org/wiki/Don%27t_Make_Me_Think) style design problem — something you should adjust for rather than legislate against? Executable specs adjust for it. Markdown specs legislate against it. BDD understood this, which is why it pushed for specs that *run*. SDD inherits the same vulnerability when its specs are just markdown files that an agent reads rather than tests that fail.

**Matching ceremony to task.** Whether you think carefully before writing code is one choice. Whether you route that thinking through a particular tool's pipeline is another. You can have BDD's discipline without surrendering visibility into what the agent is actually being told. Should you be handwriting code, using autocomplete, chatting with a model in a side pane, letting an agent run your terminal, or farming work out to agents in the cloud? People ask this as if the answer is categorical. It isn't. It's always about the task at hand. The SDD paper nods at this, but the framing still leans toward spec-everything. In large organisations, that makes sense — you want legibility, you want process that makes people somewhat interchangeable on tasks. But if you watch the desire lines, you see people reaching for the simplest approach that can work. A one-line config change doesn't need a spec. A greenfield service probably does. Matching ceremony to task size is something people do intuitively when you let them.
