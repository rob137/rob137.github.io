...
Tools that formalise SDD, like Amazon's Kiro, introduce an intermediary layer between you and the model. When you run a `/specify` command, text gets added to the context window that you don't control and can't see. If you're already comfortable with LLMs, that's noise. If you're not, you're learning two things at once: how the model works and how the tool works, with an opaque layer between them. A tight grip on what's in the context window is the difference between knowing what you're doing and watching something happen on your screen.

These are separate choices. Whether you think carefully before writing code is one. Whether you route that thinking through a particular tool's pipeline is another. You can have BDD's discipline without surrendering visibility into what the agent is actually being told.

There's a deeper problem too. Markdown specs are loosely coupled to the code, but they're also loosely coupled to the conversations that produced them. In most teams, the canonical document is what people actually said in the meeting. Tickets are a lossy shadow of that. A spec file in a repo or a Confluence page is another degree of indirection further out. Someone has to remember to update it, and it doesn't take much drift before people stop trusting it. I've seen this happen on projects — the specs exist, they're just wrong enough that nobody checks them. Written specifications that aren't executable are always one missed update from irrelevance. BDD understood this, which is why it pushed for specs that *run*. SDD inherits the same vulnerability when its specs are just markdown files that an agent reads rather than tests that fail.

There's a broader question here. When something consistently goes wrong because of human error, are you witnessing error or nature? If people reliably forget to update the spec, is the answer more rules and process, putting the responsibility on the person who forgot? Or does it reveal a [Don't Make Me Think](https://en.wikipedia.org/wiki/Don%27t_Make_Me_Think) style design problem — something you should adjust for rather than legislate against? Executable specs adjust for it. Markdown specs legislate against it.

And even when specs are current, there's the question of what the model actually does with them. People's mental models of LLM reliability vary wildly. I've seen the full range among experienced users: some assume the model will pick up anything in a spec, others assume it'll pick up more or less nothing. The truth moves depending on which model you're using, how much context it's holding, how far through a session you are. A small difference in how faithfully a model follows a spec might not matter for one task. But specs feed into code that feeds into more code, and small inaccuracies compound. A 98% reliable executor and a 99% reliable executor look almost identical on any given step. Over fifty steps, the 99% executor still has a 60% chance of getting everything right. The 98% executor is down to 36%. Over a hundred steps: 37% versus 13%. One percentage point, and before you know it you're looking at a threefold difference in outcomes.

There's also a question of scale. Should you be handwriting code, using autocomplete, chatting with a model in a side pane, letting an agent run your terminal, or farming work out to agents in the cloud? People ask this as if the answer is categorical. It isn't. It's always about the task at hand and whether the tool is appropriate for it. The SDD paper nods at this, but the framing still leans toward spec-everything. In large organisations, that makes sense — you want legibility, you want process that makes people somewhat interchangeable on tasks. But if you watch the desire lines, you see people reaching for the simplest approach that can work. A one-line config change doesn't need a spec. A greenfield service probably does. Matching ceremony to task size is something people do intuitively when you let them.
