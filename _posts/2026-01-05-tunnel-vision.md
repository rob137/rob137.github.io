...
Simon Willison used to say that an agent is basically a for loop with some tools and LLM calls that determine whether it's time to finish. When you describe it that way, it feels like a mirage. A user-facing fiction that doesn't really describe the underlying thing. There isn't an entity at all - just a process that comes and goes.
